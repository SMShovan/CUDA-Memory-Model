...
// Tiled shared memory kernel for matrix transpose
__global__ void transposeSharedMem(const float* input, float* output, int rows, int cols) {
...
int main() {
    ...
    // Launch kernel
    timer.reset();
    transposeSharedMem<<<gridDim, blockDim>>>(d_input, d_output, rows, cols);
    ...
    // Print results
    printf("Shared Memory Tiled Matrix Transpose Results:\n");
