...
// Shared memory kernel with bank conflict resolution
__global__ void transposeNoBankConflicts(const float* input, float* output, int rows, int cols) {
    ...
    // Write tile to output with bank conflict resolution
    for (int j = 0; j < TILE_DIM; j += BLOCK_ROWS) {
        if (x < rows && y + j < cols) {
...
int main() {
    ...
    // Launch kernel
    timer.reset();
    transposeNoBankConflicts<<<gridDim, blockDim>>>(d_input, d_output, rows, cols);
    ...
    // Print results
    printf("Bank Conflict-Free Matrix Transpose Results:\n");
