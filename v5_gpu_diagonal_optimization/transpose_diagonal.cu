...
// Diagonal block traversal kernel for matrix transpose
__global__ void transposeDiagonal(const float* input, float* output, int rows, int cols) {
    __shared__ float tile[TILE_DIM][TILE_DIM + 1];
    
    // Diagonal block indexing
    int blockRow = blockIdx.x;
    int blockCol = (blockIdx.x + blockIdx.y) % gridDim.x;
    
    int x = blockCol * TILE_DIM + threadIdx.x;
    int y = blockRow * TILE_DIM + threadIdx.y;
    
    // Load tile into shared memory with diagonal access pattern
    ...
    // Calculate transposed indices with diagonal pattern
    x = blockRow * TILE_DIM + threadIdx.x;
    y = blockCol * TILE_DIM + threadIdx.y;
    
    // Write tile to output with diagonal access pattern
    for (int j = 0; j < TILE_DIM; j += BLOCK_ROWS) {
        if (x < rows && y + j < cols) {
...
int main() {
    ...
    // Launch kernel
    timer.reset();
    transposeDiagonal<<<gridDim, blockDim>>>(d_input, d_output, rows, cols);
    ...
    // Print results
    printf("Diagonal Access Pattern Matrix Transpose Results:\n");
